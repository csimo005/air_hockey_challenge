...
from air_hockey_challenge.utils.tournament_agent_wrapper import (
    RemoteVsLocalTournamentAgentWrapper,
    SimpleTournamentAgentWrapper,
    RemoteTournamentAgentWrapper,
)
from baseline.ppo_baseline_agent.agent import PPOBaselineAgent
...
PENALTY_POINTS = {
    ...
    "computation_time_middle": 1,
    "computation_time_major": 2,
}
    ...
    **kwargs,
    ...
    Args:
        ...
        build_agent_2 ((mdp_info, **kwargs) -> Agent, None): Function that returns agent_2 given the env_info and
            **agent_2_kwargs. If None the PPOBaselineAgent is used.
    ...
    def agent_builder(
        ...
        if build_agent_2 is None:
            agent_2 = PPOBaselineAgent(mdp.env_info, 2)
        else:
...
def run_remote_vs_local_tournament(
    ...
    def agent_builder(mdp, i, docker_name_1, build_agent_2, agent_2_kwargs):
        ...
        host_config_1 = client.api.create_host_config(
            port_bindings={port_1: port_1}, auto_remove=True
        )
        container_id_1 = client.api.create_container(
            BASE_NAME_IMG + docker_name_1,
            detach=True,
        ...
        thread = Thread(
            target=log_docker_out,
            daemon=True,
            args=(log_dir, container_1, docker_name_1),
        )
        thread.start()
        ...
        return RemoteVsLocalTournamentAgentWrapper(
            host_1, port_1, container_id_1, name_1, log_dir, agent_2
        )
    ...
    bits, stats = container.get_archive(
        "/src/2025-challenge/air_hockey_agent/agent_config.yml"
    )
...
def run_remote_tournament(
    ...
    def agent_builder(mdp, i, docker_name_1, docker_name_2):
        ...
        host_config_2 = client.api.create_host_config(
            port_bindings={port_2: port_2}, auto_remove=True
        )
        ...
        container_id_2 = client.api.create_container(
            BASE_NAME_IMG + docker_name_2,
            detach=True,
            # network_disabled=True,
            ports=[port_2],
        ...
        def log_docker_out(
            log_dir, container_1, container_2, docker_name_1, docker_name_2
        ):
            while True:
                try:
                    container_1.reload()
                    with open(
                        os.path.join(log_dir, f"out_{docker_name_1}.log"), "wb"
                    ) as f:
                        for line in container_1.logs(stream=True, follow=False):
                            f.write(line)
                    ...
                    container_2.reload()
                    with open(
                        os.path.join(log_dir, f"out_{docker_name_2}.log"), "wb"
                    ) as f:
        ...
        thread = Thread(
            target=log_docker_out,
            daemon=True,
            args=(log_dir, container_1, container_2, docker_name_1, docker_name_2),
        )
        thread.start()

        return RemoteTournamentAgentWrapper(
            host_1,
            port_1,
            host_2,
            port_2,
...
def _run_tournament(
    ...
    **kwargs,
    ...
    data = Parallel(n_jobs=n_cores)(
        delayed(_run_single_tournament)(
            log_dir,
            agent_builder,
            name_1,
            name_2,
    ...
    for i in range(len(data)):
        ...
        result_1["Games"][f"Game-{i}"]["Winner"] = ["Draw", name_1, name_2][
            data[i][2]
        ]
        result_1["Games"][f"Game-{i}"]["Final score"] = {
            name_1: data[i][0][0],
            name_2: data[i][0][1],
        }
        result_1["Games"][f"Game-{i}"]["Goals scored"] = {
            name_1: data[i][0][0] - data[i][1][1] // 3,
            name_2: data[i][0][1] - data[i][1][0] // 3,
        }
        result_1["Games"][f"Game-{i}"]["Faults"] = {
            name_1: data[i][1][0],
            name_2: data[i][1][1],
        }
        result_1["Games"][f"Game-{i}"]["Constraint penalty points"] = data[i][3]
        result_1["Games"][f"Game-{i}"]["Constraint violation summary"] = data[i][5][
            "Violation Summary"
        ]
    ...
    if save_away_data:
        ...
        for i in range(len(data)):
            ...
            result_2["Games"][f"Game-{i}"]["Winner"] = ["Draw", name_1, name_2][
                data[i][2]
            ]
            result_2["Games"][f"Game-{i}"]["Final score"] = {
                name_1: data[i][0][0],
                name_2: data[i][0][1],
            }
            result_2["Games"][f"Game-{i}"]["Goals scored"] = {
                name_1: data[i][0][0] - data[i][1][1] // 3,
                name_2: data[i][0][1] - data[i][1][0] // 3,
            }
            result_2["Games"][f"Game-{i}"]["Faults"] = {
                name_1: data[i][1][0],
                name_2: data[i][1][1],
            }
            result_2["Games"][f"Game-{i}"]["Constraint penalty points"] = data[i][4]
            result_2["Games"][f"Game-{i}"]["Constraint violations"] = data[i][6][
                "Violation Summary"
            ]
...
def _run_single_tournament(
    ...
    **kwargs,
    ...
    mdp = AirHockeyChallengeWrapper(
        "tournament", interpolation_order=interpolation_order
    )
    ...
    # dataset, score, faults, constraints_dict_1, constraints_dict_2, computation_time[:, 0], computation_time[:, 1], \
    #            penalty_sum_1, penalty_sum_2, violations_1, violations_2
    (
        dataset,
        score,
        faults,
        ...
        violations_1,
        violations_2,
    ) = compute_metrics(core, eval_params)
    ...
    logger.log_numpy_array(computation_time=computation_time_1, **constraints_dict_1)
    ...
    if save_away_data:
        ...
        logger.log_dataset(dataset)
        logger.log_numpy_array(
            computation_time=computation_time_2, **constraints_dict_2
        )
...
def compute_metrics(core, eval_params):
    ...
    penalty_sum_1, violations_1 = get_violations(
        n_steps, episode_size, constraints_dict_1, computation_time[:, 0]
    )

    penalty_sum_2, violations_2 = get_violations(
        n_steps, episode_size, constraints_dict_2, computation_time[:, 1]
    )
    ...
    for i in range(0, n_steps, episode_size):
        for name in constraints_dict.keys():
            if np.any(np.array(constraints_dict[name][i : i + episode_size]) > 0):
                ...
                violations["Episode " + str(i / episode_size)].append(
                    name + " violated"
                )
...
if __name__ == "__main__":
    run_remote_tournament(
        "agent1", "agent1", "logs", n_episodes=1, n_cores=1, render=True, quiet=False
    )
